                        ...
                        if os.path.isdir(appdir):
                            ...
                            if len(secondDirList)==0:
                                if self.endReachCount>0:#最后一张图片已经来过，而且当前文件夹下已经没有文件了
                                    for strtmp in self.tmpWrapImageList:
                                        tmp = json.loads(strtmp)
                                        if tmppackname==tmp["packName"]:
                                            emptydir = os.path.join(self.dirPath,tmppackname)
                                            self.redisImageTask.pushPicTask(strtmp)
...
class GetTitleInfo(threading.Thread):
    def __init__(self):
        ...
        self.redisCombineTask = Task("infocombineque")
    def getTask(self):
    ...
    def run(self):
        ...
        while True:
            ...
            self.redisCombineTask.pushTask()
...
class GetImageTask(threading.Thread):
    ...
    def getTask(self):
        ...
        wrapimage=ImageWrap(bim,wrap["packName"],wrap["imageSeq"])
        return wrapimage
    def run(self):
    ...
    def run(self):
        while True:
            for value in self.taskmap.values():
                ...
                packname=""
                try:
                    packname=self.returnQue.get(timeout=5)
                    if packname <> "":
                        self.recycleTaskMap(packname)
                        time.sleep(2)
                except:
...
class ShuffleTreeBuildTask(threading.Thread):
        ...
        def run(self):
            ...
            taskrecycle.start()
            while True:
                strwrap = self.redisCombineTask.getTask()
                strwrap= json.loads(strwrap)
                wrap=WrapInfo(strwrap["packName"],strwrap["info"],strwrap["seq"])
                packname = wrap.getPackName()#根据包名来分配APK的任务，进行洗牌
                seq = wrap.getSeq()
                log.debug("ShuffleTreeBuildTask get wrap seq=%s from combineQue----"%seq)
                global picsum
                if seq=='e-n-d':
                    piccountstr=self.picWrapSumTask.getTask()
                    picWrapCount = json.loads(piccountstr)
                    picsum=picWrapCount["picSum"]
                    if self.shuffleMap[packname][2]>=picsum:
                        self.shuffleMap[packname][0].put(wrap)
                        log.info("send e-n-d wrapinfo to xml")

                    else:
                        self.shuffleMap[packname][3]=wrap#把e-n-d结点保存起来
...
class XmlCreateUtil(threading.Thread):
    ...
    def saveAsFile(self, packname):
        ...
        appruntime = packname + "_" + timeStr + "_" + str(endtime) + "\n"
...
class OcrThread(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            infowrapstr = json.dumps(wrapinfo, default=lambda obj: obj.__dict__, ensure_ascii=False)
            self.redisCombineTask.pushTask(infowrapstr)
